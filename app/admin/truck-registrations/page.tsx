...
export default function TruckRegistrationsAdmin() {
  const router = useRouter()
  const [admin, setAdmin] = useState<AdminUser | null>(null)
  const [registrations, setRegistrations] = useState<TruckRegistration[]>([])
  const [filteredRegistrations, setFilteredRegistrations] = useState<TruckRegistration[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [selectedRegistration, setSelectedRegistration] = useState<TruckRegistration | null>(null)
  ...
  const fetchTruckRegistrations = async () => {
    try {
      setIsLoading(true)
      const response = await fetch('/api/admin/truck-registrations')
      if (response.ok) {
        const data = await response.json()
        setRegistrations(data.registrations || [])
      } else {
        toast.error('Failed to fetch truck registrations')
      }
    } catch (error) {
      toast.error('Error fetching truck registrations')
  ...
  const applyFilters = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(registration => 
        ...
        registration.plateNumber.toLowerCase().includes(searchTerm.toLowerCase()) ||
        registration.truckMake.toLowerCase().includes(searchTerm.toLowerCase()) ||
        registration.truckModel.toLowerCase().includes(searchTerm.toLowerCase())
      )
    }
  ...
  const updateRegistrationStatus = async (registrationId: string, newStatus: string) => {
    try {
      setIsUpdating(true)
      const response = await fetch(`/api/admin/truck-registrations/${registrationId}`, {
        method: 'PATCH',
        headers: {
      ...
      if (response.ok) {
        ...
        fetchTruckRegistrations() // Refresh the list
  ...
  const exportToCSV = () => {
    const headers = [
      'Owner Name', 'Company', 'Email', 'Phone', 'Truck Make', 'Truck Model', 
      'Year', 'Plate Number', 'Truck Type', 'Capacity', 'Status', 'Created At'
    ]
    ...
    const csvData = filteredRegistrations.map(registration => [
      ...
      registration.email,
      registration.phone,
      registration.truckMake,
      registration.truckModel,
      registration.truckYear,
      registration.plateNumber,
      registration.truckType,
      registration.capacity || 'N/A',
      registration.status,
    ...
    a.href = url
    a.download = `truck-registrations-${new Date().toISOString().split('T')[0]}.csv`
    a.click()
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading truck registrations...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="flex-grow mx-auto max-w-7xl px-6 py-8 lg:px-8">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Truck Registrations</h1>
              <p className="mt-1 text-gray-600">
                Review and approve truck owner registrations
              </p>
            </div>
            <div className="flex items-center space-x-4">
              <button
                onClick={fetchTruckRegistrations}
        ...
        <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search by owner, company, email, plate number..."
                value={searchTerm}
        ...
        <div className="bg-white shadow overflow-hidden sm:rounded-md">
          {filteredRegistrations.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="mt-2 text-sm font-medium text-gray-900">No truck registrations found</h3>
              <p className="mt-1 text-sm text-gray-500">
                {registrations.length === 0 
                  ? "No truck registrations have been submitted yet." 
                      ...
                      <div className="mt-2 sm:flex sm:justify-between">
                        <div className="sm:flex">
                          <p className="flex items-center text-sm text-gray-500">
                            ...
                            {registration.truckMake} {registration.truckModel} ({registration.truckYear})
                          </p>
                        </div>
                        <div className="mt-2 flex items-center text-sm text-gray-500 sm:mt-0">
                          <span className="flex-shrink-0 mr-1.5 text-gray-400">
                            Plate: {registration.plateNumber}
                          </span>
                        </div>
                      </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-900">{selectedRegistration.businessType}</p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-900">{selectedRegistration.truckMake}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Model</label>
                      <p className="text-sm text-gray-900">{selectedRegistration.truckModel}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Year</label>
                      <p className="text-sm text-gray-900">{selectedRegistration.truckYear}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Plate Number</label>
                      <p className="text-sm text-gray-900">{selectedRegistration.plateNumber}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">VIN Number</label>
                      <p className="text-sm text-gray-900">{selectedRegistration.vinNumber || 'Not provided'}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Truck Type</label>
                      <p className="text-sm text-gray-900">{selectedRegistration.truckType}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Capacity</label>
                      <p className="text-sm text-gray-900">{selectedRegistration.capacity || 'Not specified'}</p>
                    </div>
                  </div>
                </div>
